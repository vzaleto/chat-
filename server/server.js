...
wss.on('connection', async (ws) => {
    ...
    ws.on('message', async (jopa) => {
        const data = JSON.parse(jopa);
        ...
        if (data.token) {
            try {
                ...
                activeConnect.set(username, ws) // test
                ...
                if (data.type === "message") {
                    ...
                    const recipientWs = activeConnect.get(data.receiver) // ada
                    if (recipientWs && recipientWs.readyState === WebSocket.OPEN) {
                        recipientWs.send(JSON.stringify({type: 'new_message', message: newMessage}));
                    }
    ...
    ws.on('close', () => {
        ...
        for (const [username, connection] of activeConnect.entries()) {
            if (connection === ws) {
                activeConnect.delete(username);
                break
            }
        }
    });
